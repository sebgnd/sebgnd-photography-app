...
export const Recent: FunctionComponent = () => {
  ...
  useEffect(() => {
    if (!loading) {
      /**
     * Delay to let reached be updated while the images are rendered.
     * Once the images are feched, there is a small time when loading
     * is false and reached is still true which causes another batch
     * of images to be feched.
     */
        ...
        img.type === 'unknown' ?
          ...
          (
            <RecentImage
              ...
              categoryDisplayName={
                categoryMap[img.categoryId]?.displayName || 'Unknown'
              }
